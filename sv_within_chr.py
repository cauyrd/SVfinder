'''
File: sv_within_chr.py
Author: Rendong
Description: detect intra-chromosomal translocation supported by pairs mapped to different chromosome
...
if len(sys.argv)<2:
	print "Usage:python sv_within_chr.py longisize.txt output.txt read_len(optional, default 100) search_len(optional, default 1000) cluster_cutoff(optional, default 1)"
	sys.exit(0)
elif len(sys.argv) == 3:
	read_len = 100
	search_len = 1000
...
for i,line in enumerate(ifp):
	...
	if items[6] == '=':
		mate_chr = items[2]
		mate_pos = int(items[7])
	else:
		mate_chr = items[2]
		mate_pos = int(items[6])
	...
	if items[2] == left_range[0] and (int(items[3]) >= left_range[1] and int(items[3]) <= left_range[2]+search_len):	#left end match
		if mate_chr == right_range[0] and intersect(right_range,mate_pos):	#right end match
			cluster.append(i+1)
			left_range[2] = int(items[3])+read_len
			continue
	if len(cluster) >= cut_off:
		print >> ofp,'intra-chromosome\t'+str(len(cluster))+'\t'+','.join(map(str,cluster))+'\t'+'\t'.join(map(str,left_range))+'\t'+'\t'.join(map(str,right_range))
	cluster = [i+1]
	left_range = [items[2],int(items[3]),int(items[3])+read_len]
	right_range = [mate_chr,mate_pos,mate_pos+read_len]
